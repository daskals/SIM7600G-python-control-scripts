...
def modem_init(pin):
    ...
    at_answer = send_at(at_command, at_expected_answer, 1)
    ...
    at_command = 'AT+CGREG?'
    # This command is used to control the presentation of an unsolicited result code +CGREG: <stat> when <n>=1
    # and there is a change in the ME network registration status, or code +CGREG: <stat>[,<lac>,<ci>] when
    # <n>=2 and there is a change of the network cell.
    at_expected_answer = '+CGREG: 0,1'
    at_answer = send_at(at_command, at_expected_answer, 0.5)
    if at_answer != 0:
        print('Result code +CGREG: ', at_answer)
    else:
        modem_reset()
